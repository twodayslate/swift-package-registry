...
function buildTermsConditions (Op, terms) {
  const andConditions = []
  if (!Array.isArray(terms)) {
    return andConditions
  }
  terms.forEach(function (term) {
    const condition = buildTermCondition(Op, term)
    if (condition) {
...
function buildTermCondition (Op, term) {
  if (!term) {
    return null
  }

  if (term.type === 'or') {
    const orConditions = buildTermsConditions(Op, term.terms)
  ...
  if (term.type === 'group') {
    const groupConditions = buildTermsConditions(Op, term.terms)
...
module.exports = function (router) {
  router.get('/search', async function (req, res) {
    ...
    const andStatement = []
    const termConditions = buildTermsConditions(Op, terms)
