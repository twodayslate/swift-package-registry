/* eslint-disable */
const { runCommand, asyncRunCommand } = require('./docker')
const Docker = require('dockerode')
const docker = new Docker()
const { Octokit } = require('@octokit/rest')
const util = require('util')

const dockerTags = ['swift:5.3', 'swift:5.2', 'swift:5.1', 'swift:5.0', 'swift:4.2', 'swift:4.1', 'swift:4.0', 'swift:3.1', 'swift:latest']
...
var robotPackage = async function (_package, callback) {
  ...
  return new Promise(async function (resolve, reject) {
    ...
    try {
      ...
      let release
      const refs = [_package.info.default_branch]
      ...
      const cb = async function (err, swiftDump, toolsVersion, description, dependencies) {
...
var processSwiftPackage = async function (docker_tag, branch, clone_url) {
  return new Promise(async function (resolve, reject) {
    ...
    try {
      ...
      const container = await docker.createContainer({ Image: docker_tag, Cmd: ['/bin/bash'], AutoRemove: true, Tty: true })
