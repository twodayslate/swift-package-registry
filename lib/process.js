...
const docker_tags = ["swift:5.2", "swift:5.1", "swift:5.0", "swift:4.2", "swift:4.1", "swift:4.0", "swift:3.1", "swift:latest"];
...
var robotPackage = async function(package, callback) {
    ...
    if (!package.info) {
        ...
        personal_access.repos.get({owner: owner, repo: repo}).then(async function(repository) {
    ...
    personal_access.repos.listReleases({owner: package.info.owner.login, repo: package.info.name }).then(async function(releases) {
        ...
        personal_access.repos.listTopics({owner: package.info.owner.login, repo: package.info.name }).then(async function(topics) {
            package.topics = topics.data.names
            package.save()

            var cb = async function(err, swift_dump, tools_version, description, dependencies) {
                if(err) {
            ...
            var loop_cb = async function(err, swift_dump, tools_version, description, dependencies) {
                if(err) {
                  index = index +1;
    ...
    function runCommandAndCheck(cmd, dir, cb) {
        runCommand(_container, cmd, dir, function(err, stdout, stderr, inspect_data) {
            ...
            if(inspect_data.ExitCode != 0) {
                if(inspect_data.ProcessConfig.entrypoint == "timeout" && inspect_data.ExitCode == 124) {
                    console.log("Got a timeout!", cmd, stdout, stderr, inspect_data)
