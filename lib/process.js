      ...
      var loopCallback = async function (err, swiftDump, toolsVersion, description, dependencies) {
        if (err) {
          ...
          if (index < dockerTags.length) {
            return processTarball(dockerTags[index], tarball, loopCallback)
          } else {
            return cb(err)
          }
        }
...
var realParsePackageQueue = async function (queue) {
  if (processingCount < 0) { // this should be in a lock
    processingCount = 0
  }
  ...
  try {
    ...
    if (_package) {
      return robotPackage(_package, function (err) {
        ...
        realParsePackageQueue(queue)
      })
    }
  } catch (error) {
    console.log('got an error processing package queue', error)
    processingCount--
  }
}
...
module.exports = {
  ...
  parsePackageQueue: async function (queue) {
    return realParsePackageQueue(queue)
  },
