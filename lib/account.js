    ...
    req.robot.auth().then(async function (octokit) {
      ...
      octokit.apps.listInstallations().then(async function (apps) {
        ...
        octokit.request('POST /app/installations/:installation_id/access_tokens', {
          ...
          const installationId = app.id

          req.oauth.apps.listInstallationReposForAuthenticatedUser({ installation_id: installationId, per_page: pageSize, page: currentPage }).then(function (repos) {
